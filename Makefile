...
base.tar:
	...
	docker run --name elementarywsl --net=host elementary/docker:stable /bin/bash -c "echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections; apt-get update; apt-get install -y -q apt-utils; apt-get full-upgrade -y -q; apt-get install -y -q apt-transport-https iproute2 aria2 bash-completion build-essential ca-certificates curl dialog htop lsof software-properties-common sudo tree; touch /etc/wsl.conf; unminimize; apt-get autoremove -y; apt-get clean;"
