    ...
    select yn in "Yup" "Nope"; do
        case $yn in
            Yup)
                ...
                printf ${ylw}"ElementaryWSL will restart after disk has been resized!!\n"${txtrst}
                sleep 2
        ...
        Yup)
            ...
            while read -p "Please enter the username you wish to create : " username; do
                if [ x$username = "x" ]; then
                    ...
                    cp ~/shutdown.cmd /mnt/c/Users/Public
                    echo " "
                    printf ${ylw}"\r\033[KTo set the new user as the default user, ElementaryWSL will terminate in 2 seconds and restart!!!"${txtrst}
                    sleep 2
                    rm ~/.bash_profile
                    powershell.exe -command "Start-Process -Verb Open -FilePath 'shutdown.cmd' -WorkingDirectory 'C:\Users\Public' -WindowStyle Hidden"
                    exec sleep 0
