...
if [ ! -e ~/.hushlogin ] && env | grep "WT_SESSION" >/dev/null 2>&1; then
    ...
    echo -e ${mgn}"You are running ElementaryWSL in Windows Terminal on $osname\n"${txtrst}
    echo -e "ElementaryWSL has been test run only in Windows command prompt on Windows 10. Windows Terminal displays an error message \033[31m[process exited with code 1 (0x00000001)]\033[0m whenever WSL is shutdown or terminated from within. This error is displayed during the disk resize and user creation steps. Though this doesn't affect the working of ElementaryWSL in any way, some of you may feel otherwise and report errors. I suggest you complete the initial setup in a Windows command prompt and switch to windows terminal post setup. If you choose to use Windows Terminal, ignore the displayed error message.\n" | fold -sw $width
