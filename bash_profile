...
if [ "$disksize" -le 274877906944 ]; then
	...
	select yn in "Yes" "No"; do
		...
		Yes)
			...
			secs=3
			printf ${ylw}"\nPlease grant diskpart elevated permissions when requested. ElementaryWSL2 will restart after disk resize.\n"${txtrst}
	...
	Yes)
		...
		while read -p "Please enter the username you wish to create : " username; do
			if [ "x$username" = "x" ]; then
				...
				secs=3
				printf ${ylw}"\nTo set the new user as the default user, ElementaryWSL will shutdown and restart!!!\n\n"${txtrst}
				while [ $secs -gt 0 ]; do
					printf "\r\033[KShutting down in %.d seconds. " $((secs--))
					sleep 1
				done
